...
int seqsearch1(struct yezhu_type yz[],int n,char kname);
...
int  funprint(struct yezhu_type yz[5000],int *p)
{
	...
	for (i=0;i<n;i++)
    {
     ...
     if ((fp=fopen("C:\\Users\\gwq12\\Desktop\\课设\\业主信息.txt","a"))==NULL)
      {
...
//       fprintf(fp,"%d %s %s %d %s %d %d %d %d",yz[i].number,yz[i].name,yz[i].phone,yz[i].home,yz[i].unite,yz[i].ele,yz[i].water,yz[i].hfee,yz[i].wfee);

   }
   fclose(fp);
   printf("请输入要查找的业主姓名：\n");
   scanf("%s",a);
  // while (a[20]==yz[i].name[20])
  // {
       if (a[20]==yz[i].name[20])
       {
           printf("业主编号 业主名称 业主电话  业主房号 业主单位  用电量 用水量 物业费 水电费\n");
           printf("%03d %11s %14s %5d %10s %8d %5d %5d %5d",yz[i].number,yz[i].name,yz[i].phone,yz[i].home,yz[i].unite,yz[i].ele,yz[i].water,yz[i].hfee,yz[i].wfee);
           i++;
       }
       else
       {
...
void output(struct yezhu_type yz[],int n)
{
   ...
   i++;
    ...
    for (i=0;i<n+9;i++)
    {
        printf("%03d %11s %14s %5d %10s %8d %5d %5d %5d\n",yz[i].number,yz[i].name,yz[i].phone,yz[i].home,yz[i].unite,yz[i].ele,yz[i].water,yz[i].hfee,yz[i].wfee);
